...
CREATE TABLE t_role (
                        ...
                        name_kz VARCHAR(255) UNIQUE NOT NULL,
...
ALTER TABLE t_recommendations
    ADD COLUMN microdistrict VARCHAR(100);

ALTER TABLE t_user
    ADD COLUMN profile_photo_url TEXT,
ADD COLUMN birth_date DATE,
ADD COLUMN microdistrict_id INT REFERENCES t_microdistrict(id),
ADD COLUMN street VARCHAR(255);

-- Индексы для быстрого поиска
CREATE UNIQUE INDEX unique_user_item ON t_recommendations (user_id, recommended_item_id);
CREATE INDEX idx_filter_category ON t_filter (category_id);
CREATE INDEX idx_item_filter ON t_item_filter (item_id, filter_id);
CREATE INDEX idx_rental_item ON t_rental (item_id);
CREATE INDEX idx_rental_status ON t_rental_status_history (rental_id);
CREATE INDEX idx_availability_calendar ON t_availability_calendar (item_id);
