<template>
  ...
  <v-container class="checklist-container">
    ...
    <v-card flat class="header-card mb-6 px-4 py-6">
      <v-card-title class="text-h4 font-weight-bold d-flex align-center">
        <v-icon color="primary" size="40" class="mr-3">mdi-clipboard-check</v-icon>
        My Inventory Checklist
      </v-card-title>
      <v-card-subtitle class="text-subtitle-1 text-grey-darken-1">
        Manage and review your inventory items
      </v-card-subtitle>
    </v-card>
    ...
    <v-row>
      ...
      <!-- Items Column -->
      <v-col cols="12" md="9">
        <v-card flat elevation="0" class="mb-4 px-2">
          <v-card-title class="text-h5 font-weight-medium d-flex align-center mb-4 px-2">
          ...
          <v-card-text class="px-0">
            ...
            <v-row v-else-if="items.length > 0">
              <v-col
                ...
                md="4"
              >
                <v-card class="item-card" elevation="2" rounded="xl" :width="300">
                  <!-- Image -->
                  <div class="image-container">
                    ...
                    >
                      <v-chip
                        :color="item.available ? 'green-lighten-5' : 'orange-lighten-5'"
                        variant="flat"
                        size="small"
                        class="ma-2 font-weight-bold"
                      >
                        {{ item.available ? 'Check Item' : 'Booked' }}
                      </v-chip>
                    </v-img>
                  </div>

                  <!-- Info -->
                  <v-card-title class="text-subtitle-1 font-weight-bold px-4 pt-3">
                    {{ item.name }}
                  </v-card-title>
                  ...
                  <v-card-text class="px-4">
                    <div class="d-flex align-center mb-1">
                      <v-icon color="primary" size="18" class="mr-1">mdi-currency-kzt</v-icon>
            ...
            >
              ...
              <div class="text-h6 font-weight-medium mb-1">No items found</div>
              <div class="text-body-2">Try adjusting your filters or select a different category</div>
            </v-alert>
          </v-card-text>
        </v-card>
...
const fetchChecklistItems = async () => {
  try {
    ...
    const response = await axios.post('/api/items/checkIn', payload, {
      headers: {
...
const viewDetails = (item) => {
  router.push(`/my-check-items/${item.id}`);
};
